...
const minifyOpts = {
  ...
  mangle: {
    ...
    properties: {
      ...
      debug: false,
      keep_quoted: true,
      regex: /^__|^(?:[A-Z_]+|prevState|state|_(?:width|height|mines|playMode|toReveal|flags|stateChange|minedCells|flagged|hasMine|revealed|touching(?:Flags|Mines)|getSurrounding|grid|subscribe|unsubscribe|reset|restart))$/
      /* Unmaintainable? *
      reserved: [
        // require.js
        "require",
...
export const terser = {
  ...
  renderChunk(code, chunk, outputOpts) {
    ...
    if (laterOut !== -1) {
      ...
      laterOut = setTimeout(() => console.log('Minify name cache: %O', nameCache, 100));
    }
    if (result.error) {
